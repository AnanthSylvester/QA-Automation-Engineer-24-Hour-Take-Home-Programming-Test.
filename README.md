...
Flow: Login → Add Item → Cart → Checkout → Order Complete
...
ResultSet rs = DBUtils.executeQuery( "SELECT * FROM orders WHERE order_id='" + orderId + "'" );
...
while(retries > 0) { ResultSet rs = DBUtils.executeQuery(query); if(rs.next()) break; Thread.sleep(2000); retries--; }
